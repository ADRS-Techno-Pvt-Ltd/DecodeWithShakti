...
// Needs the raw request body + Node crypto (HMAC signature verification) — no edge runtime.
 ...
 * unknown orders and already-finalized ones — so Razorpay stops retrying.
 * 401 only on bad signature; 500 only on our own unexpected failure (so
 * Razorpay *does* retry). Mirrors /api/v1/payment/cashfree/webhook/route.ts.
 */
export async function POST(request: Request) {
    ...
    await prisma.paymentEvent
      .create({
        data: {
          ...
          // No dedicated event-id header exists on Razorpay webhooks (unlike
          // Cashfree's x-idempotency-key) — and with a bad signature we can't
          // trust the body enough to derive one either, so always a fresh id.
  ...
  // case) — check Order first, and only fall back to the existing single-item
  // Purchase lookup when it's not one.
  ...
  if (order) {
    ...
    try {
      ...
      await prisma.paymentEvent
        ...
        .catch(() => {});
      return new Response("internal error", { status: 500 }); // let Razorpay retry
    }
