...
- **Download** (`GET /api/v1/files/download/[purchaseId]`): verifies `purchase.userId === session.user.id && purchase.status === 'SUCCESS'`, then runs `lib/watermark.ts` over the original file **in memory** — student's email diagonally on every page — and streams the watermarked bytes. The clean original on Cloudinary is never modified. Invoice download (`GET /api/v1/files/invoice/[invoiceId]`, unwatermarked — it already identifies the buyer) is allowed for the buyer **or any admin**, so admins can pull any invoice from the sales table.
